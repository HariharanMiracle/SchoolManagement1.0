...
<div style = "margin-left: 2%; background-color: #2aa5d8; border-radius: 10px;margin-top: ">
<div style= "margin-left: 20px; margin-top:20px; margin-bottom:20px;padding-top:20px; padding-bottom:20px">
<%
Connection con;
Class.forName("com.mysql.jdbc.Driver");
con=(Connection)DriverManager.getConnection("jdbc:mysql://localhost:3306/schoolmanagement", "root", "");
PreparedStatement ps=(PreparedStatement)con.prepareStatement("SELECT Title, Grade FROM subjects WHERE Teacher='" +  session.getAttribute( "uid" ) + "'");
ResultSet rs=ps.executeQuery();
while(rs.next()){ %>
	
	
<form action = "MaForm1.jsp" method="post">

  <div class="form-group row">
 
    <div class="col-sm-10">
    <input type="text" readonly class="form-control-plaintext"  name="subject"  id="staticEmail" value="Grade: <%=rs.getString("Grade")%>">
    <input class="form-control" type="text" value="<%=rs.getString("Title")%>" placeholder="Readonly input here…" readonly>
      
      
    </div>
  </div>
   <input type="submit" class="btn btn-primary" value="Manage"/></form>
